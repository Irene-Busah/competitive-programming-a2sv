...
You are suggested to automatize the process of changing the words with abbreviations. At that all too long 
words should be replaced by the abbreviation and the words that are not too long should not undergo any changes.
...
for _ in range(numOfTestCases):
    ...
    if len(word) <= 10:
        ...
        newWord = word[0] + f"{len(word[1:len(word)-1])}" + word[-1]
        print(newWord)
