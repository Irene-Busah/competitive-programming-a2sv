...
The second line of each test case contains a string s consisting of only uppercase 
or lowercase Latin characters.

Output
For each test case, output "YES" (without quotes) if s satisfies the condition, and 
...
for _ in range(numOfTestCases):
    ...
    if sorted(actualName.lower()) == sorted(name.lower()) and name.count('T') == 1 and name.replace('T', '').islower():
        print("YES")
    else:
        print("NO")
