...
on a single line. The first value is a friend's name, and the second value is an S-digit 
phone number.

After the n lines of phone book entries, there are an unknown number of lines of queries. 
Each line (query) contains a name to look up, and you must continue reading lines until there 
is no more input.
...
phone book; otherwise, print the full `name` and `phoneNumber` in the format name=phoneNumber.
